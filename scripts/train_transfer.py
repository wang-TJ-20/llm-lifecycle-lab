"""使用共享 SFT 引擎训练或恢复 Qwen Full/LoRA。

Train or resume Qwen full-parameter or LoRA SFT through the shared engine.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python scripts/train_transfer.py", description=__doc__
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_transfer_sft(
            load_run_config(args.config),
            run_id=args.run_id,
            resume_run=args.resume_run,
            resume_checkpoint=args.resume_checkpoint,
        )
        print(json.dumps(asdict(result.result), ensure_ascii=False, indent=2))
