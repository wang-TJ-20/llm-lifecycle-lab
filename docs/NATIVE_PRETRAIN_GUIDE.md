...
Checkpoint 已保存 FP16 GradScaler 和 MPS RNG；精确恢复等价测试目前仍只覆盖
CPU 路径，不能据此承诺全部设备和精度逐位一致。
...
> 本节是旧 Reference 机制示例，配置中的 1-epoch 预算已废弃。当前训练不得执行
> 本节命令，统一转到 [Base 从零训练手册](./BASE_TRAINING_GUIDE.md)。
